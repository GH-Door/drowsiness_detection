...
<p>
  ...
  Pipeline: Zoom 화면 → YOLO 탐지 → OCR 이름 인식 → FaceMesh 분석 → 룰 기반 졸음 판별 → Gradio 대시보드
</p>

<p>
  <img src="https://img.shields.io/badge/Python%203.12%2B-3776AB?style=flat-square&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white">
  <img src="https://img.shields.io/badge/YOLO11n-EE4C2C?style=flat-square&logo=ultralytics&logoColor=white">
  <img src="https://img.shields.io/badge/MediaPipe-00BCD4?style=flat-square&logo=google&logoColor=white">
  ...
  <img src="https://img.shields.io/badge/uv-6E40C9?style=flat-square">
</p>

<video src="https://github.com/user-attachments/assets/d287bf6d-72d4-4b5d-bf00-0e59d2d92b05" controls width="100%"></video>

**※ 개인정보 보호를 위해 일부 얼굴을 모자이크 처리하였습니다.**
...
| <img src="https://github.com/GH-Door.png" width="160"> | <img src="https://github.com/adhoc0909.png" width="160"> | <img src="https://github.com/LYUHAERIM.png" width="160"> |
|:---:|:---:|:---:|
...
온라인 수업 환경에서 강사의 수강생 집중도 실시간 파악 불가로 인한 교육 효과 저하 문제를 해결  
Zoom 기반 영상에서 수강생의 **졸음(DROWSY)** / **하품(YAWN)** / **이탈(ABSENT)** 상태를 자동 감지 하여, 강사에게 실시간 참여도 피드백을 제공하는 AI 모니터링 시스템

**주요 성과:**
- **YOLO11n** 커스텀 학습 — 자체 수집·라벨링 1,500장, mAP50 = **0.993**
- **3계층 졸음 판별** — FaceMesh EAR 기반 복합 조건 + 웹캠 각도 대응 Pose fallback
- **CLAHE 전처리** — 저조도 환경 랜드마크 검출율 **21%** 개선
- **프레임당 평균 처리 속도** — 92ms → 30.1ms, **62.3%** 단축 (실시간성 확보)
...
| **📊 Dataset** | 자체 수집 Zoom 화면 영상 (1,500장 직접 라벨링) |

---

## 📊 Results

YOLO11n 커스텀 모델 최종 학습 결과 — 자체 수집 Zoom 화면 데이터셋 (3 Class)
...
- 프레임 당 평균 처리 속도: 92ms → 30.1ms, **62.3%** 단축 (실시간성 확보)
- FaceMesh 병렬 처리: ThreadPoolExecutor (최대 5 workers)
- CLAHE 저조도 전처리 적용 시 얼굴 랜드마크 검출율: **80.2%** (기존 대비 약 **21% 개선**)
...
**Pipeline:** `웹캠/영상` → `썸네일 탐지` → `슬롯 매칭` → `이름 인식` → `얼굴 랜드마크 추출` → `3계층 졸음 판별` → `대시보드/PDF`  
**감지 상태:** `NORMAL` | `DROWSY` | `ABSENT` | `IGNORE` | `NOT FOUND` _(noface 시 시각화 전용 표시)_
...
| 계층 | 조건 | 설명 |
|:-----|:-----|:-----|
| **Level 1** | EAR < baseline × 0.75 + 보조 신호 1개↑ | FaceMesh 랜드마크 기반 복합 조건 |
| **Level 2** | face_center_y 급락 + Pose fallback | 수강생마다 웹캠 각도가 달라 FaceMesh 실패 시 얼굴 위치·Pose로 감지 |
| **Level 3** | 모션 없음 지속 (no-face hold) | 얼굴 미감지 상태에서 모션 홀드 |

**보조 신호:** MAR > 0.50 (하품), pitch_like (고개 끄덕임), tilt_deg > 15° (고개 기울임), PERCLOS ≥ 0.15 (30초 눈 감김 비율)
...
├── assets/                   # 데모 리소스
├── figmaApp/                 # TypeScript/React 프로토타입
└── pyproject.toml
```
...
| 졸음 임계값 | `scripts/infer_video.py` → `DrowsinessConfig` |
...
- [CVAT](https://github.com/cvat-ai/cvat) — 데이터 라벨링 도구
