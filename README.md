...
<img src="data/pipeline.png" width="90%">

</div>

**Pipeline:** `웹캠/영상` → `썸네일 탐지` → `슬롯 매칭` → `이름 인식` → `FaceMesh (EAR/MAR/pitch)` → `3계층 졸음 판별` → `Gradio 대시보드 / PDF 저장`
...
**보조 신호:** MAR > 0.50 (하품), pitch_like 급증 (고개 끄덕임), tilt_deg > 15° (고개 기울임), PERCLOS ≥ 0.15 (30초 눈 감김 비율)
