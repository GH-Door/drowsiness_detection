...
| 계층 | 조건 | 설명 |
|:-----|:-----|:-----|
| **L1** | EAR < baseline×0.75 + 보조신호 1↑ | FaceMesh 랜드마크 기반 복합 조건 |
| **L2** | face_center_y↓ + Pose fallback | 웹캠 각도 차이로 FaceMesh 실패 시 Pose로 보완 |
| **L3** | 모션 없음 지속 (no-face hold) | 얼굴 미감지 상태에서 모션 홀드 |
